...
@Stateless
@WebService(
        portName = "OTAServicesPort",
        serviceName = "OTAServicesService",
        targetNamespace = "http://www.opentravel.org/OTA/2003/05"
)
@SOAPBinding(parameterStyle = SOAPBinding.ParameterStyle.BARE)
@HandlerChain(file = "handlers.xml")
public class OTAServicesBean implements OTAServicesBeanLocal, OTAServicesBeanRemote {

    @Resource
    private javax.xml.ws.WebServiceContext webServiceContext;
    private static final org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(OTAServicesBean.class);

    public OTAServicesBean() {
    }
    ...
    public OTAHotelAvailNotifRS OTAHotelAvailNotifRQ(
        ...
        final Set<EntryQueueItem> items = new HashSet<>();
        try {
            //Validating request
            ...
            try {
                ...
                for (AvailStatusMessageType item : availStatusMessages.getAvailStatusMessage()) {
                    ...
                    final Date end = statusApplicationControl.getEnd();
                    final String invTypeCode = statusApplicationControl.getInvTypeCode();
                    items.add(new EntryQueueItem(invTypeCode, start, end));
                    ...
                    if (minStay != null && !EnumDataValueType.OWN.equals(inventory.getMinStay().getValueType())) {
                        otaHotelAvailNotifRS = new OTAHotelAvailNotifRS(new ErrorsType(new ErrorType(OTAConstants.Error.Type.NO_IMPLEMENTATION,
                                OTAConstants.Error.Code.INVALID_VALUE, getMSG("OTA_HotelAvailNotifRQ/AvailStatusMessages/" +
                                        "AvailStatusMessage/LenghtsOfStay/LengthOfStay/MinMaxMessageType with value 'SetMinLOS'",
                                invTypeCode
                        )
                        )), echoToken, version);
                        return otaHotelAvailNotifRS;
                    ...
                    if (maxStay != null && !EnumDataValueType.OWN.equals(inventory.getMaxStay().getValueType())) {
                        otaHotelAvailNotifRS = new OTAHotelAvailNotifRS(new ErrorsType(new ErrorType(OTAConstants.Error.Type.NO_IMPLEMENTATION,
                                OTAConstants.Error.Code.INVALID_VALUE, getMSG("OTA_HotelAvailNotifRQ/AvailStatusMessages/" +
                                        "AvailStatusMessage/LenghtsOfStay/LengthOfStay/MinMaxMessageType with value 'SetMaxLOS'",
                                invTypeCode
                        )
                        )), echoToken, version);
                        return otaHotelAvailNotifRS;
            ...
            try {
                integrationBeanLocal.storeSingleConnection(hotelTicker,
                        ChannelTicker.SITEMINDER, ChannelConnectionType.OFFER_UPDATE_AVAILABILITY_ARI, requestStatus
                        , items, otaHotelAvailNotifRQ, otaHotelAvailNotifRS);
//                final Integer id = integrationBeanLocal.storeSingleConnection(hotelTicker,
//                        ChannelTicker.SITEMINDER, ChannelConnectionType.OFFER_UPDATE_AVAILABILITY_ARI, requestStatus, items);
...
//                logger.info(requestCommunicationId + otaHotelAvailNotifRQ);
//                logger.info(responseCommunicationId + otaHotelAvailNotifRS);
    ...
    public OTAHotelRateAmountNotifRS OTAHotelRateAmountNotifRQ(
        ...
        try {
            ...
            final OTAHotelRateAmountNotifRS otaHotelRateAmountNotifRSInternalError = new OTAHotelRateAmountNotifRS(new ErrorsType(new ErrorType(OTAConstants.Error.Type.PROCESSING_EXCEPTION, OTAConstants.Error.Code.UNABLE_TO_PROCESS, "Internal Error. Plase try later.")),
                    otaHotelRateAmountNotifRQ.getEchoToken(), version);
            try {
                dBConnection = new DBConnection(DBProperties.getDBCustomerByTicker(hotelTicker));
                if (dBConnection == null) {
                    otaHotelRateAmountNotifRS = new OTAHotelRateAmountNotifRS(new ErrorsType(new ErrorType
                            (OTAConstants.Error.Type.PROCESSING_EXCEPTION, OTAConstants.Error.Code.NO_HOTEL_MATCH_FOUND, "hotel_code: " + hotelTicker + " not match with any of ours hotels.")),
                            otaHotelRateAmountNotifRQ.getEchoToken(), version
                    );
                    return otaHotelRateAmountNotifRS;
                ...
                for (final RateAmountMessageType rateAmountMessageType : otaHotelRateAmountNotifRQ.getRateAmountMessages().getRateAmountMessage()) {
                    ...
                    if (rate != null) {
                        ...
                        if (rate != null && !EnumDataValueType.OWN.equals(inventory.getRate().getValueType())) {
                            otaHotelRateAmountNotifRS = new OTAHotelRateAmountNotifRS(new ErrorsType(new ErrorType(OTAConstants.Error.Type.NO_IMPLEMENTATION,
                                    OTAConstants.Error.Code.INVALID_VALUE, getMSG("OTAHotelRateAmountNotifRS/RateAmountMessages/" +
                                            "RateAmountMessage/Rates/Rate/BaseByGuestAmts/BaseBuGuestAmt ",
                                    hotelTicker
                            )
                            )), echoToken, version);
                            return otaHotelRateAmountNotifRS;
                        ...
                        final DailyValuesDBHandler dailyValuesDBHandler = new DailyValuesDBHandler(inventoryDBHandler, start, end);
                        final RangeValue<Float> ratesRangeValue =
                                (rate == null ? null : dailyValuesDBHandler.getFullRatesByTicker(invTypeCode));
                        //Updating RageValues
                        Date dateIterator = new Date(start.getTime());
...
//                final String requestCommunicationId = integrationBeanLocal.generateRequestCommunicationId(id);
//                final String responseCommunicationId = integrationBeanLocal.generateResponseCommunicationId(id);
//                logger.info(requestCommunicationId + otaHotelRateAmountNotifRQ);
//                logger.info(responseCommunicationId + otaHotelRateAmountNotifRS);
            } catch (IntegrationException e) {
                logger.error(e);
            }
        }
    ...
    public OTAHotelResNotifRQ getOTAHotelResNotifRQ(final String reservationId,
        ...
        for (final HotelReservationsType.HotelReservation hotelReservation : otaHotelResNotifRQ.getHotelReservations().getHotelReservation()) {
            if (hotelReservation != null) {
                hotelReservation.setUniqueID(reservationId);
                    ...
                    for (RoomStaysType.RoomStay roomStay : hotelReservation.getRoomStays().getRoomStay()) {
                        if (roomStay.getRoomTypes() != null && roomStay.getRoomTypes().getRoomType() != null) {
                            for (final RoomTypeType roomTypeType : roomStay.getRoomTypes().getRoomType()) {
                                if (roomTypeType.getNumberOfUnits() != null && roomTypeType.getNumberOfUnits() > 1) {
...
//                            throw new NonexistentValueException("");
                    ...
                    try {
                        ...
                        for (Inventory item : inventoryDBHandler.getInventoryForScraping()) {
            ...
            return otaHotelAvailRSInternalError;
        } finally {
            try {
                integrationBeanLocal.storeSingleConnection(hotelTicker,
...
//                final Integer id = integrationBeanLocal.storeSingleConnection(hotelTicker,
//                        ChannelTicker.SITEMINDER, ChannelConnectionType.OFFER_INVENTORY, requestStatus, null);
//                final String requestCommunicationId = integrationBeanLocal.generateRequestCommunicationId(id);
//                final String responseCommunicationId = integrationBeanLocal.generateResponseCommunicationId(id);
//                logger.info(requestCommunicationId + otaHotelAvailRQ);
//                logger.info(responseCommunicationId + otaHotelAvailRS);
            } catch (IntegrationException e) {
                logger.error(e);
            }
        }
    ...
    private ErrorsType validLoginBySoapHeader(final String hotelTicker) {
        try {
            logger.error("validLoginBySoapHeader");
...
//            logger.debug(hotelTickersList);
            ...
            if (jsonResponse == null || jsonResponse.isJsonNull()) {
                ...
                for (Iterator iterator = ((JsonArray) jsonResponse).iterator(); iterator.hasNext(); ) {
                    String next = ((JsonElement) iterator.next()).getAsString();
